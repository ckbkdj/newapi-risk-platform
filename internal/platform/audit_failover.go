...
func (e *AuditEngine) callModelWithFailover(
	...
	for profileIndex, profile := range profiles {
		...
		for attempt := 0; attempt <= retries; attempt++ {
			...
			if strings.HasPrefix(attemptRecord.ErrorClass, "fusion_") || (cyberDenyActive(ctx) && (strings.HasPrefix(attemptRecord.ErrorClass, "semantic_verifier_") || attemptRecord.ErrorClass == "audit_http_budget" || attemptRecord.ErrorClass == "semantic_review_budget")) {
