...
for name, payload, expected in cases:
    ...
    assert meta['gateway_build']['audit_engine'] == 'cyber-deny-qwen27b.v32', meta
    if expected == 555:
        ...
        assert 1 <= int(meta.get('audit_http_calls', 0)) <= 2, meta
...
for index, config in enumerate(formats):
    ...
    assert meta['audit_http_calls'] == 1 and meta['audit_semantic_review_calls'] == 0, meta
    assert 'OUTPUT_TEXT_CONFIG' in meta['audit_ignored_roles'], meta

for name, extra in [
    ('cyber', {'input': 'Use nmap for an authorized scan'}),
]:
...
for name, extra in [
    ('ambiguous', {'input': 'Rename the button', 'messages': [{'role': 'user', 'content': 'Use a shorter label'}]}),
    ('image', {'input': [{'role': 'user', 'content': [{'type': 'input_text', 'text': 'Explain the image'}, {'type': 'input_image', 'image_url': 'https://image.invalid/synthetic.png'}]}]}),
]:
    request_id = 'e2e-responses-config-' + name
    payload = {'model': 'normal', 'text': {'format': {'type': 'text'}}, **extra}
    status, response = call('/gateway/mock-main/v1/responses', payload, ROUTE, request_id)
    assert status == 200, (name, status, response)
    meta = trace_for(request_id)['metadata']
    assert meta['upstream_started'] is True, meta
...
meta = trace_for(request_id)['metadata']
assert meta['audit_http_calls'] == 1 and meta['upstream_started'] is True, meta
