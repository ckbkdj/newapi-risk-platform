...
old = "\t\t\tTimeout:   30 * time.Second,\n"
new = "\t\t\t// Per-request contexts enforce the profile or long-context timeout.\n\t\t\t// A fixed client timeout would cancel 262K prefills before they finish.\n\t\t\tTimeout: 0,\n"
if old not in audit:
    raise SystemExit("audit HTTP client timeout anchor not found")
audit_path.write_text(audit.replace(old, new, 1), encoding="utf-8")

doc = r'''# Qwen3.8 长上下文快速审计
...
'''
(root / "docs/qwen38-fast-audit.md").write_text(doc, encoding="utf-8")

print("Qwen3.8 audit follow-up fixes applied")
