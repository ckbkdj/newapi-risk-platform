...
# v31 clean allow fast path: no deterministic weak signal means one model call.
assert qm["audit_model_confidence"] is None
assert qm["audit_http_calls"] == 1, "clean qualitative allow must remain single-pass"
assert qm["audit_semantic_review_calls"] == 0, "clean qualitative allow unexpectedly invoked verifier"
assert qm["audit_output_contract"] == "risk_audit_output.v2"
assert qm["gateway_build"]["audit_engine"] == "cyber-deny-qwen27b.v32"
...
BASE_URL="${BASE_URL}" RISK_ADMIN_TOKEN="${TOKEN}" ROUTE_KEY="${ROUTE_KEY}" python3 scripts/e2e-audit-db-sync.py
...
BASE_URL="${BASE_URL}" RISK_ADMIN_TOKEN="${TOKEN}" ROUTE_KEY="${ROUTE_KEY}" python3 scripts/e2e-audit-v14.py

BASE_URL="${BASE_URL}" RISK_ADMIN_TOKEN="${TOKEN}" ROUTE_KEY="${ROUTE_KEY}" python3 scripts/e2e-audit-v15.py
