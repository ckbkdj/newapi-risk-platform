...
启用的 Cyber 规则命中直接返回 HTTP 555，包括旧 `review/allow` 动作；自动化测试、授权、安全研究和内部调试不提供豁免。模型输出的有效拒绝不能被复核/Fusion 推翻。普通未触发 Cyber 的请求经两次短结构化检查后放行；未覆盖输入或审计失败也拒绝，但单独标记为审计不可用。详见 [千问3.8-27B 审计规范与验收](docs/cyber-deny-qwen27b.md)。
...
修复凭据工程例外的机械放行、续写目标裁剪、空/未覆盖输入默认放行，以及初判 allow 绕过必要复核的问题。**多模态和 `previous_response_id` 尚未实现完整审计覆盖；开启 fail-closed 后这些输入会明确返回覆盖错误，而不是静默放行。** 升级前请阅读 [行为变化、回归与验收说明](docs/audit-intent-coverage-20260909.md)。
...
### v13：正常开发判定与 Maven/Android 误拦修复

正常仓库认证、依赖下载、配置与文档不能仅凭凭据字段/路径被判窃密；修复详情、验收边界和真实审计模型的只审不转发评估见 [normal-development-v13.md](docs/normal-development-v13.md)。v13 保留 HTTP 555 的真实禁用操作和审计失败保护，不增加整请求白名单，不修改管理员规则或数据库配置。
