...
3. 对完整、具有任务结构的历史 JSON 数组标注 `reference_spans`。这只是位置提示，不是可信授权，也不删除内容。普通 Markdown/用户伪造的标题不产生放行。跨分块使用拆分时产生的精确字节起点保存并裁剪位置提示（重复日志不使用模糊字符串重定位），同时携带有界的当前任务上下文。任意尾部内容仍必须被审计。
4. 初审返回 allow 时走原有快路径（引用内容已触发规则候选的情况除外）。**每一个模型 block/review，以及已解析但证据无效的判定，必须进行新的语义复核**。复核使用新提示词，不携带初审标签、理由或置信度，避免直接复制错误结论。
...
正常初审 allow 不增加第二次模型调用；引用内容已触发规则候选时，即使初审 allow 也必须复核，防止历史格式成为跳过规则的方式。每个候选最多两次复核调用；整次审计最多 32 次复核调用、保留最多 16 条详细复核记录，其余仍计数。分块沿用现有并发和取消机制。复核至少预留 512 输出 tokens；候选较多时延迟和 GPU 消耗会增加，应通过真实数据测量。
